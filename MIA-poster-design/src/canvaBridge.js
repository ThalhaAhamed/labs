...
export function createCanvaBridge(apiKey, options = {}) {
  ...
  child.once('exit', (code) => markStopped(new Error(`Canva bridge stopped with code ${code}.`)));
  child.once('error', (error) => markStopped(new Error(`Could not start Canva bridge: ${error.message}`)));
