# MIA Poster Design Docker Quickstart
...
For a complete guide for one runtime, choose [Node.js](HOW_TO_RUN_NODE.md) or [Docker Compose](HOW_TO_RUN_DOCKER.md).
...
Before running, configure the saved MeetStream Hosted Agent with the exact dashboard settings in the [README prerequisites](README.md#meetstream-dashboard-prerequisites). The MCP URL and authorization header are the only values that must wait until the setup bridge is running.
...
`MCP_AUTH_DIR` is normally `$HOME/.mcp-auth` on macOS or Linux and `$env:USERPROFILE\.mcp-auth` on Windows. Use the expanded absolute path rather than the literal variable name. Never commit `.env`.
...
For the complete native workflow, use [Run MIA Poster Design with Node.js](HOW_TO_RUN_NODE.md).
...
| Port `29865` already in use | Stop the other local MIA/Canva bridge process, then retry. |
